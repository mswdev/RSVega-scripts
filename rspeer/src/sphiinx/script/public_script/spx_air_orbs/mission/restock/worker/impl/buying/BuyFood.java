...
public class BuyFood extends AirOrbRestockWorker {

    public boolean has_item;
    private final int jug_of_wine_id = 1993;
    private final OpenGrandExchange open_grand_exchange;

    public BuyFood(AirOrbRestockMission mission) {
        super(mission);
    ...
    @Override
    public void work() {
        final int amount_in_bank = BankCache.getValue(jug_of_wine_id);
        final int amount_to_buy = 5000 - amount_in_bank;
        if (amount_to_buy <= 0) {
            has_item = true;
            return;
        }
        ...
        if (GrandExchange.isOpen()) {
            if (GrandExchangeSetup.isOpen() && GrandExchangeSetup.getItemId() == jug_of_wine_id) {
                ...
                if (GrandExchange.getView() == GrandExchange.View.BUY_OFFER) {
                    if (GrandExchangeSetup.setItem(jug_of_wine_id)) {
                        Time.sleepUntil(() -> GrandExchangeSetup.getItemId() == jug_of_wine_id, 1500);
                    }
                } else {
