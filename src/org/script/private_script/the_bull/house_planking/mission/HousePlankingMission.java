...
public class HousePlankingMission extends Mission implements ItemManagement {
    ...
    public ItemManagementEntry[] itemsToBuy() {
        return new ItemManagementEntry[]{
                new ItemManagementEntry(this, RING_OF_WEALTH_IDS[5], 5, () -> getScript().getBankCache().getBankCache().getOrDefault(RING_OF_WEALTH_IDS[1], 0) <= 0 && getScript().getBankCache().getBankCache().getOrDefault(RING_OF_WEALTH_IDS[2], 0) <= 0 && getScript().getBankCache().getBankCache().getOrDefault(RING_OF_WEALTH_IDS[3], 0) <= 0 && getScript().getBankCache().getBankCache().getOrDefault(RING_OF_WEALTH_IDS[4], 0) <= 0 && getScript().getBankCache().getBankCache().getOrDefault(RING_OF_WEALTH_IDS[5], 0) <= 0 && Inventory.getCount(RING_OF_WEALTH_IDS[5] + 1) <= 0),
                ...
                new ItemManagementEntry(this, getLogType().getItemId(), getQuantity(), () -> getScript().getBankCache().getBankCache().getOrDefault(getLogType().getItemId(), 0) <= 0 && Inventory.getCount(true, getLogType().getItemId()) <= 0 && Inventory.getCount(getLogType().getNotedItemId()) <= 0 && !House.isInside() && !shouldEnd),
                new ItemManagementEntry(this, RuneType.LAW.getItemId(), ((getQuantity()) / 24) * 2 + 1, () -> getScript().getBankCache().getBankCache().getOrDefault(RuneType.LAW.getItemId(), 0) <= 0 && Inventory.getCount(true, RuneType.LAW.getItemId()) <= 0 && !shouldEnd),
                new ItemManagementEntry(this, RuneType.EARTH.getItemId(), ((getQuantity()) / 24) * 2 + 1, () -> getScript().getBankCache().getBankCache().getOrDefault(RuneType.EARTH.getItemId(), 0) <= 0 && Inventory.getCount(true, RuneType.EARTH.getItemId()) <= 0 && !shouldEnd)
        };
    }
