package antiban.api.game.pricechecking;
...
public class PriceChecking {
    ...
    public static int getRSPrice(int id) {
        try (BufferedReader READER = new BufferedReader(new InputStreamReader(new URL("http://services.runescape.com/m=itemdb_oldschool/antiban.api/catalogue/detail.json?item=" + id).openStream()))) {
    ...
    public static int getOSBuddyPrice(int id) {
        try (BufferedReader READER = new BufferedReader(new InputStreamReader(new URL("https://antiban.api.rsbuddy.com/grandExchange?a=guidePrice&i=" + id).openStream()))) {
